...
async def remove_reverted_transfers(ctx: HookContext, id: int) -> None:
    ...
    try:
        ...
        for transfer in transfers:
            await update_ownership(transfer, transfer.to_address)
            await update_ownership(transfer, transfer.from_address)
...
async def update_ownership(transfer, owner):
    ...
    if owner not in null_addresses:
        logger.info(f"Updating ownership: {transfer.contract}:{transfer.token_id}:{owner}")
        await process(transfer.contract, transfer.token_id, owner, transfer.date)
