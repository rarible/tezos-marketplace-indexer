...
async def import_origination_transfers(ctx: HookContext):
    logger.info(f'Starting query_origination_transfers')
                ...
                if is_mint and is_burn:
                    ...
                    if assert_token_id_length(str(tx['token']['id'])):
                        ...
                        if to_address is not None:
                            await process(token_transfer.contract, token_transfer.token_id, to_address, token_transfer.date)
                        if from_address is not None:
                            await process(token_transfer.contract, token_transfer.token_id, from_address, token_transfer.date)
                    else:
                        logger.warning(f"tokenId is too long, len={len(str(tx['token']['id']))}, transfer id={tx['id']}")

        if len(transactions) > 0:
            last_id = transactions[-1]['id']
        else:
            break
    logger.info(f'Finished query_origination_transfers')
