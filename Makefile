...
test:
	$(py) pytest -vv $(source_dir) $(unit_tests_dir)
...
up_prod: prepare_services_prod
	docker-compose -f docker-compose.prod.yml up --build --remove-orphans --force-recreate --no-deps
	--abort-on-container-exit indexer-rarible indexer-tezos indexer-metadata
